...
// ─────────────────────────────────────────────────────────────────────────────
// LOCK SCREEN MANAGER
// Handles PIN setup, verification, and auto-lock
// ─────────────────────────────────────────────────────────────────────────────
class LockManager {
    constructor(onUnlock) {
        this.onUnlock     = onUnlock;   // callback when PIN verified
        this.sessionKey   = null;       // derived from PIN, held in memory only
        this.lockTimer    = null;
        this.isLocked     = true;
    }
    ...
    async setupPIN(pin, confirmPin) {
        ...
        // Derive the encryption key from the PIN and store session key in memory
        this.sessionKey = pin;
        return true;
    }
    ...
    async unlock(pin) {
        ...
        if (!ok) return false;
        this.sessionKey = pin;   // keep PIN in memory as encryption key
        this.isLocked   = false;
        this.resetAutoLock();
        return true;
    }
    ...
    // Re-encrypt all data with a new PIN
    async changePIN(oldPin, newPin, confirmPin, app) {
        ...
        // Re-encrypt localStorage data with new key
    ...
    showLockScreen(mode = 'unlock') {
        ...
        const handleKey = async (key) => {
            ...
            if (isSetup && phase === 'create' && buf.length >= 4) {
                // Auto-advance to confirm after short pause
                setTimeout(() => {
                    confirmBuf = '';
                    phase = 'confirm';
                    const tmp = buf; buf = '';
            ...
            if (isSetup && phase === 'confirm' && buf.length >= 4) {
                ...
                if (buf !== first) { phase='create'; hint.textContent='PINs didn\'t match — try again'; showError('PINs do not match. Start over.'); return; }
                try {
...
class StoreWiseApp {
    ...
    boot() {
        ...
        // Activity events reset auto-lock timer
    ...
    async loadFromStorage() {
        try {
            ...
            if (encrypted && this.lock.sessionKey) {
                ...
                // First run — no encrypted data yet, use defaults
            ...
            console.warn('Decrypt failed — using defaults. Wrong PIN?', e);
            this.products     = JSON.parse(JSON.stringify(this.defaultProducts));
            this.transactions = JSON.parse(JSON.stringify(this.defaultTransactions));
            this.suppliers    = JSON.parse(JSON.stringify(this.defaultSuppliers));
            this.settings     = JSON.parse(JSON.stringify(this.defaultSettings));
    ...
    async saveToLocalStorage() {
        if (!this.lock.sessionKey) return; // never write without a key
        const json      = JSON.stringify(this.getDataBundle());
        const encrypted = await SwCrypto.encrypt(json, this.lock.sessionKey);
        localStorage.setItem('sw_enc_data', encrypted);
    }
    ...
    async connectFileSystem() {
        ...
        try {
            const [handle] = await window.showOpenFilePicker({ types:[{description:'StoreWise Data',accept:{'application/json':['.json']}}] });
            this.fileHandle = handle;
            const text = await (await handle.getFile()).text();
            // File may be encrypted or plain JSON
            let data;
            try { data = JSON.parse(await SwCrypto.decrypt(text, this.lock.sessionKey)); }
            catch { data = JSON.parse(text); } // fallback: unencrypted backup
    ...
    disconnectGDrive() {
        ...
        localStorage.removeItem('sw_gdrive_token'); localStorage.removeItem('sw_gdrive_file_id'); localStorage.setItem('sw_sync_provider','local');
    ...
    restoreCloudConnections() {
        ...
        if (p==='filesystem') { this.syncProvider='local'; localStorage.setItem('sw_sync_provider','local'); } // handle can't be restored
    }
    ...
    setupEventListeners() {
        document.addEventListener('click', (e) => {
            if (e.target.closest('.menu-item')) { e.preventDefault(); const s=e.target.closest('.menu-item').getAttribute('data-section'); if(s) this.showSection(s); return; }
            const btn=e.target.closest('[id]'); if(!btn) return;
            const map = {
                quickSaleBtn:()=>this.showSection('sales'), addProductBtn:()=>this.showProductModal(), addNewProductBtn:()=>this.showProductModal(),
                closeProductModal:()=>this.hideProductModal(), cancelProductBtn:()=>this.hideProductModal(), saveProductBtn:()=>this.saveProduct(),
                closeCheckoutModal:()=>this.hideCheckoutModal(), cancelCheckoutBtn:()=>this.hideCheckoutModal(), completePaymentBtn:()=>this.completePayment(),
                clearCartBtn:()=>this.clearCart(), checkoutBtn:()=>this.showCheckoutModal(),
                addSupplierBtn:()=>this.showSupplierModal(), closeSupplierModal:()=>this.hideSupplierModal(), cancelSupplierBtn:()=>this.hideSupplierModal(), saveSupplierBtn:()=>this.saveSupplier(),
                settingsExportBtn:()=>this.exportData(), settingsImportBtn:()=>this.importData(), settingsResetBtn:()=>this.resetData(),
                exportDataBtn:()=>this.exportData(), importDataBtn:()=>this.importData(),
                saveStoreSettingsBtn:()=>this.saveStoreSettings(), saveInventorySettingsBtn:()=>this.saveInventorySettings(),
                connect_filesystem:()=>this.connectFileSystem(), disconnect_filesystem:()=>this.disconnectFileSystem(),
                connect_gdrive:()=>this.connectGDrive(), disconnect_gdrive:()=>this.disconnectGDrive(),
                connect_dropbox:()=>this.connectDropbox(), disconnect_dropbox:()=>this.disconnectDropbox(),
                gdrive_load_now:()=>this.loadFromGDrive(), dropbox_load_now:()=>this.loadFromDropbox(),
                lockAppBtn:()=>this.lock.lock(),
                changePINBtn:()=>this.showChangePINModal(), saveChangePINBtn:()=>this.doChangePIN(),
            ...
            if (e.target.classList.contains('settings-nav-item')) { const s=e.target.getAttribute('data-settings'); if(s) this.showSettingsPanel(s); }
    ...
    showSection(id) {
        ...
        ({dashboard:()=>this.renderDashboard(),inventory:()=>this.renderInventory(),sales:()=>this.renderSales(),analytics:()=>this.renderAnalytics(),suppliers:()=>this.renderSuppliers(),settings:()=>this.renderSettings()})[id]?.();
    ...
    renderDashboard() { this.updateMetrics(); this.renderStockAlerts(); this.renderRecentActivity(); setTimeout(()=>this.renderSalesChart(),200); }
    ...
    renderStockAlerts() {
        ...
        list.innerHTML=low.length===0?'<div class="alert-item"><div class="alert-message">No stock alerts ✅</div></div>':low.map(p=>`<div class="alert-item"><div class="alert-product">${p.name}</div><div class="alert-message">Only ${p.stock} left (min: ${p.min_stock})</div></div>`).join('');
    }

    renderRecentActivity() {
        const list=document.getElementById('activityList');if(!list)return;
        const r=this.transactions.slice(-5).reverse();
        list.innerHTML=r.length===0?'<div class="activity-item"><div class="activity-text">No recent activity</div></div>':r.map(t=>`<div class="activity-item"><div class="activity-time">${t.time}</div><div class="activity-text">Sale ${t.id} — $${t.total.toFixed(2)} (${t.payment_method})</div></div>`).join('');
    }
    ...
    filterProducts() {
        ...
        grid.innerHTML=f.length===0?'<div class="empty-state"><p>No products found</p></div>':f.map(p=>`
    ...
    deleteProduct(id) { const p=this.products.find(p=>p.id===id);if(!p||!confirm(`Delete "${p.name}"?`))return;this.products=this.products.filter(p=>p.id!==id);this.saveToStorage();this.updateMetrics();this.filterProducts(); }
    ...
    renderTransactions() {
        ...
        list.innerHTML=r.length===0?'<div class="empty-state"><p>No transactions yet</p></div>':r.map(t=>`<div class="transaction-item"><div class="transaction-header"><span class="transaction-id">${t.id}</span><span class="transaction-amount">$${t.total.toFixed(2)}</span></div><div class="transaction-details">${t.time} • ${t.payment_method} • ${t.customer_name||'Walk-in'}</div></div>`).join('');
    }

    handlePOSSearch(q) { if(q.length<2)return;const p=this.products.find(p=>p.name.toLowerCase().includes(q.toLowerCase())||p.barcode===q);if(p){this.addToCart(p.id);const el=document.getElementById('posSearch');if(el)el.value='';} }

    addToCart(pid,qty=1) { const p=this.products.find(p=>p.id===pid);if(!p)return;const ex=this.cart[pid];if(ex)ex.quantity+=qty;else this.cart[pid]={product_id:pid,quantity:qty,price:p.price,name:p.name};this.updateCartDisplay(); }

    get cartItems() { return Object.values(this.cart); }

    updateCartDisplay() {
        const ci=document.getElementById('cartItems'),cs=document.getElementById('cartSubtotal'),ct=document.getElementById('cartTax'),cto=document.getElementById('cartTotal'),cb=document.getElementById('checkoutBtn');
        if(!ci)return;
        const items=this.cartItems;
        if(!items.length){ci.innerHTML='<div class="empty-cart"><p>Scan or search products to add to cart</p></div>';[cs,ct,cto].forEach(el=>{if(el)el.textContent='$0.00';});if(cb)cb.disabled=true;return;}
        ci.innerHTML=items.map((item,idx)=>`<div class="cart-item"><div class="cart-item-info"><div class="cart-item-name">${item.name}</div><div class="cart-item-price">$${item.price.toFixed(2)} each</div></div><div class="cart-item-quantity"><button class="quantity-btn" onclick="window.app.updateCartQty('${item.product_id}',-1)">-</button><span style="padding:0 8px">${item.quantity}</span><button class="quantity-btn" onclick="window.app.updateCartQty('${item.product_id}',1)">+</button><button class="quantity-btn" onclick="window.app.removeFromCart('${item.product_id}')" style="margin-left:8px;color:var(--color-error)">×</button></div></div>`).join('');
        const sub=items.reduce((s,i)=>s+i.price*i.quantity,0),tax=sub*this.settings.tax_rate,total=sub+tax;
    ...
    updateCartQty(pid,ch) { if(this.cart[pid]){this.cart[pid].quantity=Math.max(1,this.cart[pid].quantity+ch);this.updateCartDisplay();} }
    ...
    showCheckoutModal() {
        ...
        const el=document.getElementById('checkoutTotal');if(el)el.textContent=`$${(sub+sub*this.settings.tax_rate).toFixed(2)}`;
        document.getElementById('checkoutModal')?.classList.remove('hidden');
    }
    hideCheckoutModal() { document.getElementById('checkoutModal')?.classList.add('hidden'); }

    async completePayment() {
        const method=document.querySelector('input[name="paymentMethod"]:checked')?.value||'cash',cname=document.getElementById('customerName')?.value||'';
        const items=this.cartItems,sub=items.reduce((s,i)=>s+i.price*i.quantity,0),total=sub+sub*this.settings.tax_rate;
        const t={id:`T${String(this.transactions.length+1).padStart(3,'0')}`,date:new Date().toISOString().split('T')[0],time:new Date().toLocaleTimeString('en-US',{hour12:false,hour:'2-digit',minute:'2-digit'}),items:[...items],total:parseFloat(total.toFixed(2)),payment_method:method,customer_name:cname||null};
    ...
    renderAnalytics() { this.updatePerformanceMetrics();setTimeout(()=>{this.renderTrendsChart();this.renderTopProductsChart();},200); }

    updatePerformanceMetrics() {
        let rev=0,cost=0;this.products.forEach(p=>{rev+=p.price*p.stock;cost+=p.cost*p.stock;});
        const gm=rev>0?(rev-cost)/rev*100:0,avg=this.transactions.length?this.transactions.reduce((s,t)=>s+t.total,0)/this.transactions.length:0,sold=this.transactions.reduce((s,t)=>s+t.items.reduce((ss,i)=>ss+i.quantity,0),0),avgS=this.products.reduce((s,p)=>s+p.stock,0)/(this.products.length||1),turn=avgS>0?sold/avgS:0,acc=this.products.length?this.products.filter(p=>p.stock>=p.min_stock).length/this.products.length*100:100;
    ...
    renderSuppliers() {
        ...
        g.innerHTML=this.suppliers.map(s=>`<div class="supplier-card"><div class="supplier-name">${s.name}</div><div class="supplier-contact">📞 ${s.contact}</div><div class="supplier-contact">✉️ ${s.email}</div><div class="product-actions" style="margin-top:12px"><button class="btn btn--outline btn--sm" onclick="window.app.editSupplier('${s.id}')">Edit</button><button class="btn btn--danger btn--sm" onclick="window.app.deleteSupplier('${s.id}')">Delete</button></div></div>`).join('');
    }

    showSupplierModal(id=null) {
        const modal=document.getElementById('supplierModal'),title=document.getElementById('supplierModalTitle'),form=document.getElementById('supplierForm');if(!modal||!title||!form)return;
    ...
    async saveSupplier(){const modal=document.getElementById('supplierModal'),id=modal?.dataset.supplierId,name=document.getElementById('supplierName')?.value.trim(),contact=document.getElementById('supplierContact')?.value.trim(),email=document.getElementById('supplierEmail')?.value.trim();if(!name){alert('Name required.');return;}if(id){const s=this.suppliers.find(s=>s.id===id);if(s)Object.assign(s,{name,contact,email});}else this.suppliers.push({id:`S${String(this.suppliers.length+1).padStart(3,'0')}`,name,contact,email});await this.saveToStorage();this.populateDropdowns();this.hideSupplierModal();this.renderSuppliers();this.showToast('✅ Supplier saved!');}
    editSupplier(id){this.showSupplierModal(id);}
    async deleteSupplier(id){const s=this.suppliers.find(s=>s.id===id);if(!s||!confirm(`Delete "${s.name}"?`))return;this.suppliers=this.suppliers.filter(s=>s.id!==id);await this.saveToStorage();this.renderSuppliers();}
    ...
    showProductModal(id=null){const modal=document.getElementById('productModal'),title=document.getElementById('productModalTitle'),form=document.getElementById('productForm');if(!modal||!title||!form)return;if(id){title.textContent='Edit Product';this.populateProductForm(this.products.find(p=>p.id===id));}else{title.textContent='Add New Product';form.reset();}modal.dataset.productId=id||'';modal.classList.remove('hidden');}
    hideProductModal(){document.getElementById('productModal')?.classList.add('hidden');}
    populateProductForm(p){if(!p)return;const f={productName:p.name,productCategory:p.category,productBarcode:p.barcode,productPrice:p.price,productCost:p.cost,productStock:p.stock,productMinStock:p.min_stock,productSupplier:p.supplier,productDescription:p.description};Object.entries(f).forEach(([id,v])=>{const el=document.getElementById(id);if(el)el.value=v;});}
    async saveProduct(){const modal=document.getElementById('productModal'),id=modal?.dataset.productId;const fd={name:this.getVal('productName'),category:this.getVal('productCategory'),barcode:this.getVal('productBarcode'),price:parseFloat(this.getVal('productPrice'))||0,cost:parseFloat(this.getVal('productCost'))||0,stock:parseInt(this.getVal('productStock'))||0,min_stock:parseInt(this.getVal('productMinStock'))||0,supplier:this.getVal('productSupplier'),description:this.getVal('productDescription')};if(!fd.name){alert('Product name required.');return;}if(id){const p=this.products.find(p=>p.id===id);if(p)Object.assign(p,fd);}else this.products.push({id:`P${String(this.products.length+1).padStart(3,'0')}`,image_url:this.getProductEmoji(fd.category),...fd});await this.saveToStorage();this.hideProductModal();this.updateMetrics();if(this.currentSection==='inventory')this.renderInventory();this.showToast('✅ Product saved!');}
    ...
    populateDropdowns(){document.querySelectorAll('#categoryFilter,#productCategory').forEach(sel=>{sel.innerHTML=(sel.id==='categoryFilter'?'<option value="">All Categories</option>':'')+this.categories.map(c=>`<option value="${c}">${c}</option>`).join('');});const sup=document.getElementById('productSupplier');if(sup)sup.innerHTML=this.suppliers.map(s=>`<option value="${s.name}">${s.name}</option>`).join('');const fields={settingsStoreName:this.settings.store_name,settingsAddress:this.settings.address,settingsPhone:this.settings.phone,settingsTaxRate:(this.settings.tax_rate*100).toFixed(2),settingsLowStockThreshold:this.settings.low_stock_threshold};Object.entries(fields).forEach(([id,v])=>{const el=document.getElementById(id);if(el)el.value=v;});}

    // ═══════════════════════════════════════════════════════════════════════════
    // HELPERS
    // ═══════════════════════════════════════════════════════════════════════════

    getTodaySales(){const t=new Date().toISOString().split('T')[0];return this.transactions.filter(t2=>t2.date===t).reduce((s,t2)=>s+t2.total,0);}

    showToast(msg,type='success',duration=3000){let t=document.getElementById('swToast');if(!t){t=document.createElement('div');t.id='swToast';t.style.cssText='position:fixed;bottom:24px;right:24px;background:var(--color-surface);border:1px solid var(--color-border);border-radius:var(--radius-base);padding:12px 20px;box-shadow:var(--shadow-lg);z-index:9999;font-size:var(--font-size-md);color:var(--color-text);transition:opacity 0.3s;opacity:0;max-width:360px;';document.body.appendChild(t);}t.textContent=msg;t.style.opacity='1';clearTimeout(this._toastTimer);this._toastTimer=setTimeout(()=>{t.style.opacity='0';},duration);}

    exportData(){const b=new Blob([JSON.stringify(this.getDataBundle(),null,2)],{type:'application/json'});const u=URL.createObjectURL(b),a=document.createElement('a');a.href=u;a.download=`storewise-backup-${new Date().toISOString().split('T')[0]}.json`;document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(u);this.showToast('✅ Data exported (unencrypted — keep this file safe!)');}

    importData(){const input=document.createElement('input');input.type='file';input.accept='.json,.enc';input.onchange=e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=ev=>{try{const d=JSON.parse(ev.target.result);if(confirm('Replace all data?')){this.applyDataBundle(d);this.showToast('✅ Data imported!');}}catch{alert('Error: Invalid file format');}};r.readAsText(file);};input.click();}

    resetData(){if(!confirm('Reset ALL data to defaults?'))return;this.products=JSON.parse(JSON.stringify(this.defaultProducts));this.transactions=JSON.parse(JSON.stringify(this.defaultTransactions));this.suppliers=JSON.parse(JSON.stringify(this.defaultSuppliers));this.settings=JSON.parse(JSON.stringify(this.defaultSettings));this.saveToStorage();this.populateDropdowns();this.updateMetrics();this.renderDashboard();this.showToast('✅ Data reset!');}

    // ═══════════════════════════════════════════════════════════════════════════
    // CHARTS
    // ═══════════════════════════════════════════════════════════════════════════

    renderSalesChart(){const canvas=document.getElementById('salesChart');if(!canvas)return;if(this.charts.sales)this.charts.sales.destroy();const days=[],sales=[];for(let i=6;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);const ds=d.toISOString().split('T')[0];days.push(d.toLocaleDateString('en-US',{weekday:'short'}));sales.push(this.transactions.filter(t=>t.date===ds).reduce((s,t)=>s+t.total,0));}this.charts.sales=new Chart(canvas.getContext('2d'),{type:'line',data:{labels:days,datasets:[{label:'Sales',data:sales,borderColor:'#1FB8CD',backgroundColor:'rgba(31,184,205,0.1)',fill:true,tension:0.4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true}}}});}

    renderTrendsChart(){const canvas=document.getElementById('trendsChart');if(!canvas)return;if(this.charts.trends)this.charts.trends.destroy();const labels=['4 wks ago','3 wks ago','2 wks ago','This week'],data=[0,0,0,0];this.transactions.forEach(t=>{const d=Math.floor((new Date()-new Date(t.date))/86400000);if(d<7)data[3]+=t.total;else if(d<14)data[2]+=t.total;else if(d<21)data[1]+=t.total;else if(d<28)data[0]+=t.total;});this.charts.trends=new Chart(canvas.getContext('2d'),{type:'bar',data:{labels,datasets:[{label:'Sales',data,backgroundColor:['#1FB8CD','#FFC185','#B4413C','#ECEBD5']}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}}}});}

    renderTopProductsChart(){const canvas=document.getElementById('topProductsChart');if(!canvas)return;if(this.charts.topProducts)this.charts.topProducts.destroy();const sm={};this.transactions.forEach(t=>t.items.forEach(i=>{sm[i.product_id]=(sm[i.product_id]||0)+i.quantity;}));const top5=this.products.map(p=>({name:p.name,sold:sm[p.id]||0})).sort((a,b)=>b.sold-a.sold).slice(0,5);this.charts.topProducts=new Chart(canvas.getContext('2d'),{type:'doughnut',data:{labels:top5.map(p=>p.name),datasets:[{data:top5.map(p=>p.sold||1),backgroundColor:['#1FB8CD','#FFC185','#B4413C','#ECEBD5','#5D878F']}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}}}});}
}
